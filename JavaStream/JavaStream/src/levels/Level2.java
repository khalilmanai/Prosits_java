...
public class Level2 {
    ...
    public static void main(String[] args) {
        ...
        double average = teachers.stream().filter(s->s.getName().startsWith("s"))./* TO DO 3 */;


        /* TO DO 4: Retourner la liste des enseignants dont le nom commence par f */
        List<Teacher> teachers1 = teachers.stream().filter(s->s.getName()
                .startsWith("f")).collect(Collectors.toList());/* TO DO 4 */;
        ...
        boolean test = teachers.stream().filter(s->s.getSalary() > 100000).anyMatch(teacher -> teacher.getSalary() > 10000);/* TO DO 6 */;
